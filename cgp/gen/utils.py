...
def reconstruct_atoms(all_reconstructed_positions, all_distances, all_angles, all_dihedral_angles, fixed_atoms, z_matrix, device=torch.device("cuda:0")):
    ...
    all_atoms_to_indices = torch.tensor(all_atoms_to_indices, device=device).long()
    all_indices_to_atoms = torch.tensor(all_indices_to_atoms, device=device).long()
    reconstruct_order = torch.tensor(reconstruct_order, device=device).long()
    current_index = fixed_atoms.shape[0]
    for reconstruct_block in reconstruct_blocks:
        reconstruct_block = torch.tensor(reconstruct_block, device=device).long()
        ...
        b = all_distances[:, reconstruct_idx].unsqueeze(-1)
        a = all_angles[:, reconstruct_idx].unsqueeze(-1)
        t = all_dihedral_angles[:, reconstruct_idx].unsqueeze(-1)
